...
export default (genNameDirty, createJS) => {
  ...
  const createPath = path.resolve(process.cwd() + '/static/src/components/' + genName + '/' + filename);
...
`;
  ...
  if (fs.existsSync(path.resolve(process.cwd() + '/static/src/components/' + genName))) {
